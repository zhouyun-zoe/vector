...
mod tests {
    ...
    use crate::{
        state, test_util,
        watcher::{mock as mock_watcher, Mock as MockWatcher},
    };
    ...
    async fn test_delayed_deletes() {
        ...
        let logic = tokio::spawn(async move {
            ...
            // the state.
            assert_eq!(
                state_events_rx.next().await.unwrap().unwrap_item(),
    ...
    async fn run_flow_test(
        ...
        let state_writer = state::evmap::Writer::new(state_writer, None); // test without debounce to avoid complexity
